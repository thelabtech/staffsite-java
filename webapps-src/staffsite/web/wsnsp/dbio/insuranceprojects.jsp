...
<p> &nbsp;
<center><%=fontXL%>WSN International Summer Project Listing for Insurance</font></center>
<p>
<table width='100%' border='1' cellpadding=3 bordercolor="<%=bordercolor%>" BORDERCOLORLIGHT="<%=bordercolorlight%>">
<tr valign='top' align='center'><td> &nbsp; </td><td nowrap><%=fontTextL%><b>Start Date</td><td nowrap><%=fontTextL%><b>Stop Date</td><td nowrap><%=fontTextL%><b>Project Name</td>
<td rowspan="30"> &nbsp;
<td rowspan="30">
	<p align='left'>This is a display of all the projects in order of when they start. You may click
	...
	if (!h.containsKey("0")) {
		...
		while (h.containsKey(counter.toString())){
			...
			<tr align=center>
				<td>
					<a href="/servlet/WsnSpController?view=insurancesubmitform&action=insuranceSubmitForm&id=<%=proj.get("WsnProjectID")%>"><img src='/wsnsp/images/magglass.gif' border='0'></a>
				</td>
				<td><%=font%><%=proj.get("StartDate")%>&nbsp;</td>
				<td><%=font%><%=proj.get("StopDate")%>&nbsp;</td>
				<td><%=font%><a href="/servlet/WsnSpController?view=insurancesubmitform&action=insuranceSubmitForm&id=<%=proj.get("WsnProjectID")%>"><%=proj.get("Name")%>&nbsp; (<%=proj.get("PartnershipRegion")%>)</a></td>
			</tr>
			<%
