...
<tr><th><%=fontS%>result</th><th><%=font%>Name</th><th><%=font%>Address</th><th><%=font%>City, State</th><th><%=font%>E-mail</th><th><%=font%>Phone</th></tr>
	...
	if (h.size()==0) {
		...
		<TH colspan=8><%=font%>There are currently no entries matching your search criteria.</font></TH>
			...
			<BR><BR>We want to make sure that all of our WSN project staff have access to the Campus Staff Website so that 
			they'll be able to access the Summer Project Tool. If the staff who you're trying to add didn't show up
			in the search above, it means that they don't have access to the Staff Site yet. 
			Please send an <a href="mailto:help@campuscrusadeforchrist.com&subject=Please create new Staff Site account for WSN User">e-mail to Customer 
			Service</a> and request a staff.campuscrusadeforchrist.com login account for the person. If the 
			person is on staff, include their Staff Account number. Also, please indicate whether they are married or single. 
			Customer Service will create a login account for the person and reply to you within 24 hours. Once Customer Service 
			notifies you that the login account has been created, try your search again by clicking "Add Project Staff" on 
			your WSN home screen, and you should be able to find them.
		<%
	} else {
		...
		while (h.containsKey(counter.toString())){
			...
			<tr align=center>
				...
				<td><%=fontText%><a href="/servlet/WsnSpController?view=appinfo&action=getStaffInfo&id=<%=formHelp.value((String)app.get("AccountNo"))%>"><%=formHelp.value((String)app.get("Name"))%></a>&nbsp;</td>
