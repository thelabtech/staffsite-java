...
%>
		<TR>
			...
			<A HREF="/servlet/Review360Controller?action=newReviewSession&section=3&revieweeId=<%=request.getParameter("revieweeId")%>&requestedById=<%=myStaff.get("AccountNo")%>"><%=fontB%><%=myStaff.get("PreferredName")%>&nbsp;<%=myStaff.get("LastName")%></A>&nbsp;(<%=myStaff.get("Address1")%>, <%=myStaff.get("City")%>, <%=myStaff.get("State")%>, <%=myStaff.get("Zip")%>)</FONT>
			</TD>
		</TR>
<%
		}
...
%>
		<%=box.printTop()%>
		<FORM NAME="submitForm" METHOD="POST" ACTION="/servlet/Review360Controller?action=listStaff">
