...
%>
		<TR>
			...
			<A HREF="/servlet/Review360Controller?action=associateReviewer&reviewSessionId=<%=request.getParameter("reviewSessionId")%>&reviewerId=<%=myStaff.get("AccountNo")%>&returnAction=<%=request.getParameter("returnAction")%>"><%=fontB%><%=myStaff.get("PreferredName")%>&nbsp;<%=myStaff.get("LastName")%></A>&nbsp;(<%=myStaff.get("Address1")%>, <%=myStaff.get("City")%>, <%=myStaff.get("State")%>, <%=myStaff.get("Zip")%>)</FONT>
			</TD>
		</TR>
<%
		}
