...
function valid(form) {
	...
	if (field.value == "") {
		...
		alert("You must enter at least 1 characters of someone's name");
		field.focus();
	} else {
		form.submit();
	}
...
<TABLE BORDER="0" CELLPADDING="0" CELLSPACING="0" WIDTH="100%" <%=bgcolorW%>> <!--Whole page table-->
    <TR>
        <TD <%=bgcolorW%> VALIGN="BOTTOM" COLSPAN="3"><IMG SRC="/images/360logo.gif" BORDER="0">&nbsp;<%=fontB5%><B>Start a new 360 Light [2/3]</B></FONT></TD>
	</TR>
	<TR>
...
<TABLE WIDTH="100%" BORDER="0" CELLPADDING="3" CELLSPACING="0"> <!-- Big frame inside table-->
	<TR>
		...
		<OL>
			...
			<%=fontR%><LI>Select the person who requested the review process for the above person.</FONT>
...
%>
		<TR>
			...
			<A HREF="/servlet/Review360LightController?action=newReviewSession&section=3&revieweeId=<%=request.getParameter("revieweeId")%>&requestedById=<%=myStaff.get("AccountNo")%>"><%=fontB%><%=myStaff.get("PreferredName")%>&nbsp;<%=myStaff.get("LastName")%></A>&nbsp;(<%=myStaff.get("Address1")%>, <%=myStaff.get("City")%>, <%=myStaff.get("State")%>, <%=myStaff.get("Zip")%>)</FONT>
			</TD>
		</TR>
<%
		}
...
%>
		...
		<FORM NAME="submitForm" METHOD="POST" ACTION="/servlet/Review360LightController?action=listStaff">
			<INPUT TYPE="hidden" NAME="lastAction" VALUE="newReviewSession2">
