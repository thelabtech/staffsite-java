...
<TABLE BORDER="0" CELLPADDING="0" CELLSPACING="0" WIDTH="100%" <%=bgcolorW%>> <!--Whole page table-->
    <TR>
        <TD <%=bgcolorW%> VALIGN="BOTTOM" COLSPAN="3"><IMG SRC="/images/360logo.gif" BORDER="0">&nbsp;<%=fontB5%><B>Edit Details</B></FONT></TD>
	</TR>
	<TR>
...
%>
		<%=box.printTop()%>
		<FORM NAME="submitForm" METHOD="POST" ACTION="/servlet/Review360LightController?action=saveReviewSessionDetails">
		...
		<INPUT TYPE="hidden" NAME="returnAction" VALUE="<%=request.getParameter("returnAction")%>">
			<TR>
				...
				<TD <%=bgcolorG%> ALIGN="LEFT"><INPUT NAME="name" MAXLENGTH="64" SIZE="50" VALUE="<%=mySession.get("Name")%>"></TD>
				<SCRIPT LANGUAGE="JavaScript">
				<!--
			...
			<TR>
				...
				<TD <%=bgcolorG%> ALIGN="LEFT"><%=fontB%><%=tub.getValue("RequestedByName")%></FONT></TD>
			</TR>
			<TR>
				<TD <%=bgcolorG%> ALIGN="RIGHT"><%=fontB%>360 Due Date</FONT></TD>
				<TD <%=bgcolorG%> ALIGN="LEFT">
					<INPUT TYPE="text" NAME="dateDue" SIZE="10" MAXLENGTH="10" onFocus="this.blur()" VALUE="<%=dateFormatter.format((Date)mySession.get("DateDue"))%>">
					&nbsp;<%=fontB%>
			...
			<TR>
				...
				<TD <%=bgcolorG%> ALIGN="LEFT"><TEXTAREA NAME="purpose" ROWS="10" COLS="50" WRAP="VIRTUAL" onKeyUp="validate(this)"><% if(mySession.get("Purpose") != null) out.print(mySession.get("Purpose"));%></TEXTAREA></TD>
			</TR>
			<!-- Hidden form values -->
