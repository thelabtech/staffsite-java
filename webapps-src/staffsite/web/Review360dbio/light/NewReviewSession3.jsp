...
<TABLE BORDER="0" CELLPADDING="0" CELLSPACING="0" WIDTH="100%" <%=bgcolorW%>> <!--Whole page table-->
    <TR>
        <TD <%=bgcolorW%> VALIGN="BOTTOM" COLSPAN="3"><IMG SRC="/images/360logo.gif" BORDER="0">&nbsp;<%=fontB5%><B>Start a new 360 Light [3/3]</B></FONT></TD>
	</TR>
	<TR>
...
%>
		<%=box.printTop()%>
		<FORM NAME="submitForm" METHOD="POST" ACTION="/servlet/Review360LightController?action=saveReviewSession">
			<TR>
				<TD <%=bgcolorG%> ALIGN="RIGHT"><%=fontB%>Name for this 360</FONT></TD>
				<TD <%=bgcolorG%> ALIGN="LEFT"><INPUT NAME="name" MAXLENGTH="64" SIZE="50" VALUE="(i.e. Spring 2002)"></TD>
				<SCRIPT LANGUAGE="JavaScript">
				<!--
			...
			<TR>
				...
				<TD <%=bgcolorG%> ALIGN="LEFT"><%=fontB%><%=tub.getValue("RequestedByName")%>[<A HREF='/servlet/Review360LightController?action=newReviewSession&section=2&revieweeId=<%=request.getParameter("revieweeId")%>'>change</A>]</FONT></TD>
			</TR>
			<TR>
