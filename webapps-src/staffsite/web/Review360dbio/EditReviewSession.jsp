...
<TABLE BORDER="0" CELLPADDING="0" CELLSPACING="0" WIDTH="100%" <%=bgcolorW%>> <!--Whole page table-->
   <TR>
        <TD <%=bgcolorW%> VALIGN="BOTTOM" COLSPAN="3"><IMG SRC="/images/360logo.gif" BORDER="0">&nbsp;<%=fontB5%><B>Administration - "<%=mySession.get("Title")%>"</B></FONT></TD>
	</TR>
	<TR>
	...
	<TR>
		...
		<TR>
			...
			<TD <%=bgcolorW%> ALIGN="RIGHT">
				<!--<%=fontR%>[<A HREF="javascript: deleteSession('<%=mySession.get("Title")%>','<%=mySession.get("reviewSessionId")%>');"><%=fontR%>Delete</FONT></A>]</FONT>--></TD>
			<TD <%=bgcolorW%> ALIGN="RIGHT"><%=fontB%>[<A HREF="/servlet/Review360Controller?action=editReviewSessionDetails&reviewSessionId=<%=mySession.get("reviewSessionId")%>&returnAction=editReviewSession"><%=fontB%>Edit</FONT></A>]</FONT></TD>
		</TR>
		<TR>
...
%>
		...
		<TR>
			...
			<TD <%=bgcolorW%> COLSPAN="4" ALIGN="CENTER"><%=fontB%>[<A HREF="/servlet/Review360Controller?action=addReviewer&reviewSessionId=<%=mySession.get("reviewSessionId")%>&returnAction=editReviewSession"><%=fontB%>Add Staff Reviewer</FONT></A>]<br />[<A HREF="/servlet/Review360Controller?action=addNonStaffReviewer&reviewSessionId=<%=mySession.get("reviewSessionId")%>&returnAction=editReviewSession"><%=fontB%>Add Non-Staff Reviewer</FONT></A>]</FONT></TD>
		</TR>
		<TR>
		...
		<FORM ACTION="/servlet/Review360Controller?action=sendEmails&lastAction=editReviewSession" NAME="email<%=counter%>" METHOD="post">
