...
<TABLE BORDER="0" CELLPADDING="0" CELLSPACING="0" WIDTH="100%" <%=bgcolorW%>> <!--Whole page table-->
    <TR>
        <TD <%=bgcolorW%> VALIGN="BOTTOM" COLSPAN="3"><IMG SRC="/images/360logo.gif" BORDER="0">&nbsp;<%=fontB5%><B>Start a new 360 [1/3]</B></FONT></TD>
	</TR>
	<TR>
...
%>
		<TR>
			...
			<A HREF="/servlet/Review360Controller?action=newReviewSession&section=2&revieweeId=<%=myStaff.get("AccountNo")%>"><%=fontB%><%=myStaff.get("LastName")%>,&nbsp;<%=myStaff.get("PreferredName")%></A>&nbsp;(<%=myStaff.get("Address1")%>, <%=myStaff.get("City")%>, <%=myStaff.get("State")%>, <%=myStaff.get("Zip")%>)</FONT>
			</TD>
		</TR>
<%
		}
