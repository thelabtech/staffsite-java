		...
		<%=box.printTop()%>
			...
			<% while(activeTargetCampusI.hasNext()) {
				...
				<TR>
					...
					<TD><%=font%>[<A HREF="/servlet/InfoBaseController?action=editActivity&locallevelid=<%= teamID %>&referrer=locallevel&targetareaid=<%= row.get("TargetAreaID") %>&strategy=<%= row.get("Strategy") %>&activityid=<%= row.get("ActivityId") %>&status=<%= row.get("Status") %>">Update</A>]</FONT></TD>
				</TR>
			<% } %>
