...
%>
 ...
 %>
	...
	if (!hasMovement) {
			%>
		<%=font%><I>There are no micro-culture movements recorded.</I>
		<BR></FONT> <%
     }
...
<BR>
<%=fontL%>&nbsp;<B>Non-Crusade Ministries:</B></FONT><BR>
				<%
				...
				<TABLE ALIGN="CENTER" BORDER=0 CELLPADDING=0 CELLSPACING=0
					...
					<TR>
						<TD>
						<P><%=font%>&nbsp;<I>There are no non-Crusade ministries
						recorded.</I></FONT>
						<P>
					</TR>
					</TD>
				...
				<TABLE ALIGN="CENTER" WIDTH="80%" BORDER=0 CELLPADDING=0
					...
					<TR>
						<TD><%=font%>[<A
							HREF="/servlet/InfoBaseController?action=addMinToCampus&targetareaid=<%= request.getParameter("targetareaid") %>">Add
						non-CCC Ministry to this campus</A>] <!--[<A HREF="/servlet/InfoBaseController?action=editNonCCCMin&mode=add&targetareaid=<%= request.getParameter("targetareaid") %>">Add a non-Crusade ministry</A>]--></FONT></TD>
					</TR>
				</TABLE>
...
<BR>
<%=fontL%>&nbsp;<B>CCC Ministries Non-Campus Ministries:</B></FONT><BR>
...
%>
	...
	if (!hasMovement) {
			%>
		<%=font%><I>There are no ccc ministry non-campus ministries
						recorded.</I>
		<BR></FONT> <%
     }
