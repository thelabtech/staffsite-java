...
ar = ActionResults.getActionResults(session);
	...
	Iterator act;
	 Integer movs=new Integer(0);
		...
		String pref = "";
		act = ar.getHashtable("activities").keySet().iterator();
			while(act.hasNext()) {
				pref = (String)act.next();
				pileUpSuccessCriteriaIds += "&activities["+pref+"]="+ar.getHashtable("activities").get(pref);
				movs++;
	...
	while(slist.hasNext()) // print dates
		{
			...
			if(weeksBack!=(15-counter))
				{ 
					%>						
					<div class="weekTab" style="left:<%=(counter*54)+20 %>px">
				...
				{
					%>
					<div class="weekBackground" style="left:<%=(counter*54)+19 %>px"></div>
					<div class="thisWeek"  style="left:<%=(counter*54)+20 %>px"><%=fontB%>
					<%=criteria.get("PeriodBeginShort")%><br>
					to<br>
					<%=criteria.get("PeriodEndShort")%><BR>
					</div>
	...
	while(slist.hasNext())
	{
		...
		{	//note the id attributes below are for a possible future improvement 
			...
			if (!mr.getValue("activityid").equals(lastMovement)){
				...
				%>
				<a style="position:absolute;top:<%= 70+errorJog+(formHeight)%>;left:20px;z-index:10;font-size:10px;color:#cacaca;" href="/servlet/InfoBaseController?action=deleteFastSuccessCriteriaBookmark&delBookmark=<%=mr.getValue("activityid")%>">remove bookmark</a><%
			}
