...
<%
	...
	if (staffInfo != null) {
		...
		strategy = (String)staffInfo.get("Strategy");
		jobStatus = (String)staffInfo.get("JobStatus");
		status = (String)staffInfo.get("CountryStatus");
		workLocation = (String)staffInfo.get("PrimaryEmpLocDesc");
	}
...
function jobVerify(form) {
	var isValid = true;
	if(document.jobChangeForm1.currentMinistry.value == "" && document.jobChangeForm1.newMinistry.value == "") {
		isValid = false;
		alert("You must choose a ministry.");
	}
	
	if(isValid && document.jobChangeForm1.ministryChange.checked && document.jobChangeForm1.newMinistry.value == "") {
		isValid = false;
	...
	if(isValid && document.jobChangeForm1.currentRegion.value == "" && document.jobChangeForm1.newRegion.value == "") {
		if (<%=ministry.equals("Campus Ministry")%> && !(document.jobChangeForm1.ministryChange.checked)) {
			isValid = false;
			alert("You must choose a region.");
		}
		else if (document.jobChangeForm1.ministryChange.checked && document.jobChangeForm1.newMinistry.value == "Campus Ministry") {
			isValid = false;
			alert("You must choose a region.");
		}
	}
	...
	if(isValid && document.jobChangeForm1.locationChange.checked && document.jobChangeForm1.newLocation.value == ""){
		isValid = false;
		alert("You must enter a new work location.");
	}
	return isValid;
}
-->
</SCRIPT>
<TITLE> Job Change - Step 1 </TITLE>
<%@ include file="hrheader.jspf" %>
<FORM NAME="jobChangeForm1" ACTION="/servlet/HRUpdateController?action=getTeamList" METHOD="POST" onSubmit="return jobVerify(document.jobChangeForm1)">
<INPUT TYPE="hidden" NAME="region" VALUE="<%=region%>">
<CENTER>
<TABLE BORDER=0 CLASS="outline" CELLPADDING=0 CELLSPACING=0>
<TR><TD>

<TABLE WIDTH="100%" CELLPADDING=0 CELLSPACING=0><TR><TD CLASS="div" ALIGN="LEFT">&nbsp;Job Change form</TD><TD ALIGN="RIGHT" CLASS="div">Date: <%= calToday.get(Calendar.MONTH) + 1%>/<%= calToday.get(Calendar.DAY_OF_MONTH)%>/<%= calToday.get(Calendar.YEAR)%></TD></TR></TABLE>
...
<TABLE WIDTH="100%" CELLPADDING=0 CELLSPACING=0><TR><TD CLASS="div">&nbsp;Position Change Information&nbsp;</TD><TD CLASS="div" ALIGN="RIGHT">&nbsp;<I>Please check the box for the appropriate category(ies) you desire to update or correct.</TD></TR></TABLE>
	<!--<TABLE WIDTH="98%" ALIGN="CENTER" BORDER=0 CELLPADDING=0 CELLSPACING=0>
		<TR>
			<TD WIDTH="100%" ALIGN="RIGHT">
				<NOBR><INPUT TYPE="checkbox" class="noborder" NAME="includeSpouse" VALUE="true"> Check if this form covers spouse</NOBR>
			</TD>
		</TR>
	</TABLE>-->
	<CENTER>
	...
	<TR>
		<TD>
			<NOBR>Change effective: <INPUT TYPE="text" SIZE="1" MAXLENGTH="2" NAME="jobChangeMonth" onKeyUp="KeyPress(this,event,2,'jobChangeForm1.jobChangeDay.focus()')" VALUE="<%= calTomorrow.get(Calendar.MONTH) + 1%>">/<INPUT TYPE="text" SIZE="1" MAXLENGTH="2" NAME="jobChangeDay" onKeyUp="KeyPress(this,event,2,'jobChangeForm1.jobChangeYear.focus()')" VALUE="<%= calTomorrow.get(Calendar.DAY_OF_MONTH)%>">/<INPUT TYPE="text" SIZE="1" MAXLENGTH="4" NAME="jobChangeYear" VALUE="<%= calTomorrow.get(Calendar.YEAR)%>"></NOBR>
		</TD>
...
<OPTION VALUE="Worldwide Challenge">Worldwide Challenge</OPTION>
		...
		<TD CLASS="outline" ALIGN="CENTER">
			...
			<TR>
				<TD CLASS="small">
					...
					<SELECT NAME="statusMenu" CLASS="small" onChange="document.jobChangeForm1.statusChange.checked=true">
					<OPTION VALUE=""></OPTION>
			...
			<TR>
				<TD COLSPAN="2" CLASS="small">
					<INPUT CLASS="noborder" TYPE="radio" NAME="newStatus" VALUE="statusOther">Other: <INPUT CLASS="small" TYPE="text" NAME="statusOther" SIZE="15" onChange="document.jobChangeForm1.statusChange.checked=true">
				</TD>
			</TR>
			</TABLE>
	...
	<TR>
		...
		<% if (region!=null) { %>
			<TD CLASS="outline" ALIGN="CENTER"><input type="hidden" name="currentRegion" value=<%=region%>><%=(String)regionLookup.get(region)%>&nbsp;</TD>
		<% } else { %>
			<TD CLASS="outline" ALIGN="CENTER">
			<SELECT NAME="currentRegion" CLASS="small" onChange="document.jobChangeForm1.regionChange.checked=true">
				<OPTION VALUE=""></OPTION>
				...
				<OPTION VALUE="IN">Impact National Office</OPTION>
			</SELECT>
			</TD>
		<% } %>
		<TD CLASS="outline" ALIGN="CENTER"><SELECT NAME="newRegion" CLASS="small" onChange="document.jobChangeForm1.regionChange.checked=true">
			<OPTION VALUE=""></OPTION>
			<OPTION VALUE="NC">National Campus Office</OPTION>
			<OPTION VALUE="NE">Northeast</OPTION>
			<OPTION VALUE="NW">Greater Northwest</OPTION>
			...
			<INPUT TYPE="checkbox" CLASS="noborder" NAME="showAllTeams" VALUE="true">
			I will need to select a team <Br>outside of my region.
		</TD>
	</TR>	
	<TR>
		...
		<TD CLASS="outline" ALIGN="CENTER"><SELECT NAME="newLane" CLASS="small" onChange="document.jobChangeForm1.laneChange.checked=true">
			...
			<OPTION VALUE="Field Ministry">Field Ministry</OPTION>
			<OPTION VALUE="Impact President's Office">Impact President's Office</OPTION>
			</SELECT>
		</TD>
	</TR>
	<TR>
		<TD CLASS="outline"><INPUT TYPE="checkbox" CLASS="noborder" NAME="locationChange" VALUE="true"><B>Work Location</B></TD>
		<TD CLASS="outline" align="center"><%=workLocation%>&nbsp;</TD><input type="hidden" name="currentLocation" value="<%=workLocation%>">
		<TD CLASS="outline" align="center"><input type="text" name="newLocation" onKeyPress="document.jobChangeForm1.locationChange.checked=true">
		<BR>City, State, Country</TD>
	</TR>
	</TABLE>
