...
<%
	...
	if (staffInfo != null) {
		...
		title = (String)staffInfo.get("SpouseTitle");
		role = (String)staffInfo.get("SpouseRole");
		team = (String)staffInfo.get("SpouseTeamName");
...
function jobVerify(form) {
	var isValid = true;
	if(document.jobChangeForm2.roleChange.checked && document.jobChangeForm2.newRole.value == "") {
		isValid = false;
...
<TITLE> Spouse Job Change - Step 2 </TITLE>
<%@ include file="hrheader.jspf" %>
<FORM NAME="jobChangeForm2" ACTION="/servlet/HRUpdateController?action=jobChangeRequest&spouse=true" METHOD="POST" onSubmit="return jobVerify(document.jobChangeForm2)">
...
<TABLE WIDTH="100%" CELLPADDING=0 CELLSPACING=0><TR><TD CLASS="div">&nbsp;Position Change Information&nbsp;</TD><TD CLASS="div" ALIGN="RIGHT">&nbsp;<I>Please check the box for the appropriate category(ies) you desire to update or correct.</TD></TR></TABLE>
	...
	<TR>
		<TD CLASS="outline"><INPUT TYPE="checkbox" CLASS="noborder" NAME="roleChange" VALUE="true"><B>Position</B></TD>
		<TD CLASS="outline" ALIGN="CENTER"><%=role%>&nbsp;</TD>
		<TD CLASS="outline" ALIGN="CENTER">
			<SELECT CLASS="small" NAME="newRole">
			<OPTION VALUE=""></OPTION>
	...
	<TR>
		...
		<% } else { %>
			<SELECT NAME="newTeam" CLASS="small">
			<OPTION VALUE=""></OPTION>
			<%
...
<!--			<OPTION VALUE="staffed">Staffed Campus</OPTION>
	...
	<TR>
		...
		<TD CLASS="outline" ALIGN="CENTER"><INPUT NAME="newJob" CLASS="small">
	</TR>
	</TABLE>
