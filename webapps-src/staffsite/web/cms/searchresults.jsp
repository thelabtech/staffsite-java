...
<TITLE>Search </TITLE>
	...
	for(int i=start; i<end; i++) {
		...
		<tr>
			<td><%=font%><%=i+1%>. </td>
			<td>
				<%=fontL%><b><a href="/servlet/CmsController?action=getFile&id=<%=result.get("CmsFileID")%>&name=/<%=result.get("Title")%>" target="_blank"><%=result.get("Title")%></a></b></font> 
				<%=font%>
		...
		<tr>
			...
			<td>
				...
				<a href="/servlet/CmsController?action=fileInfo&catReturnTo=searchResults&id=<%=result.get("CmsFileID")%>">[More Info]</a>
				<br><br>
			</td>
		</tr>
		<%
