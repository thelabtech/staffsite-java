...
function validate() {
	...
	errorMessage = "You didn't enter the following required fields:\n"
		...
		emptyCheck(document.fileForm.typeFile,"Write a story.");
...
<FORM NAME="fileForm" ACTION="/servlet/CmsController?action=popupUpload" METHOD="POST" ENCTYPE="multipart/form-data" onSubmit="return validate()">
	...
	<input type=hidden name="submitType" value="typeText">
	<TABLE BORDER="0" CELLPADDING="3" WIDTH="90%" CELLSPACING="0">
		...
		<TR>
			<TD><%=fontB%>Type story/article here:</FONT></TD>
			<TD><TEXTAREA NAME=typeFile ROWS="10" COLS="50" WRAP="VIRTUAL"></TEXTAREA></TD>
		</TR>
		<TR>
			<TD><%=fontB%>Author(s)</FONT></TD>
			<TD><INPUT NAME="Author" SIZE="50" value="<%=profile.get("FirstName")%> <%=profile.get("LastName")%>"></TD>
		</TR>
		<TR>
			<TD><%=fontB%>Contact Person Info</FONT></TD>
			<TD><INPUT NAME="Contact" SIZE="50" value="<%=profile.get("FirstName")%> <%=profile.get("LastName")%>"></TD>
		</TR>
		<TR>
		...
		<TR>
			<TD><%=fontB%>Summary (optional):</FONT></TD>
			<TD><TEXTAREA NAME="Summary" ROWS="5" COLS="50" WRAP="VIRTUAL"></TEXTAREA></TD>
		</TR>
		<TR>
			<TD><%=fontB%>Message to<BR>Moderator (optional):</FONT></FONT></TD>
