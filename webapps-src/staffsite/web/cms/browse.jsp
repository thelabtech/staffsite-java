...
function openWin(input){ 
	if(input == 'addCat'){
		window.open("/servlet/CmsController?action=showAddCategory&catId=<%=tub.get("CmsCategoryID")%>","Add","resizable=no,scrollbars=yes,width=400,height=100,top=100,left=100");
	}
	else if(input == 'removeCat'){
		window.open("/servlet/CmsController?action=showDeleteCategory&id=<%=tub.get("CmsCategoryID")%>","Remove","resizable=no,scrollbars=yes,width=400,height=100,top=100,left=100");
	}
	else if(input == 'renameCat'){
		window.open("/servlet/CmsController?action=showRenameCategory&id=<%=tub.get("CmsCategoryID")%>","Rename","resizable=no,scrollbars=yes,width=400,height=100,top=100,left=100");
	}
	else if(input == 'upload'){
		window.open("/servlet/CmsController?action=popupUploadForm&catId=<%=tub.get("CmsCategoryID")%>","AddFile","resizable=no,scrollbars=yes,width=600,height=550,top=100,left=100");
	}
}
</SCRIPT>
</HEAD>
				...
				<TR>
					<TD width='40%' valign=top>
						...
						for (int i=0;i<fs.size()/2;i++) { // The first half of files to list
							...
							%>
							<A HREF="/servlet/CmsController?action=fileInfo&id=<%=f.get("CmsFileID")%>&catReturnTo=<%=tub.get("CmsCategoryID")%>"><img src="/images/small_magglass.gif" alt='view resource details' border=0></A>
							...
							<%
								if(moderator.equals("true")) {
									...
									if (!moderated.booleanValue()) {
										%>
										<A HREF="/servlet/CmsController?action=edit&id=<%=f.get("CmsFileID")%>&catReturnTo=<%=tub.get("CmsCategoryID")%>">
