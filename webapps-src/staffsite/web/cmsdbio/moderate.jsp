...
<%
	...
	for (int i=0;i<fs.size();i++) {
		...
		<A HREF="/servlet/CmsController?action=saveReview&id=<%=f.get("FileId")%>&ModeratedYet=true">[Accept]</A>
		<A HREF="/servlet/CmsController?action=deleteResource&id=<%=f.get("FileId")%>">[Delete]</A>
		<A HREF="/servlet/CmsController?action=moderateReview&id=<%=f.get("FileId")%>">[Review]</A>
		</font>
		<br>
		<%
	}
