...
function openWin(input){ 
	if(input == 'addToCat'){
		window.open("/servlet/CmsController?action=browseCat&catReturnTo=<%=ar.getValue("CatReturnTo")%>&id=<%=f.get("FileId")%>","Add","resizable=no,scrollbars=yes,width=400,height=500,top=100,left=100");
	} else if(input == 'replace'){
		window.open("/servlet/CmsController?action=showReplace&catReturnTo=<%=ar.getValue("CatReturnTo")%>&id=<%=f.get("FileId")%>","ReplaceResource","resizable=no,scrollbars=yes,width=600,height=300,top=100,left=100");
	}
}
</SCRIPT>
</HEAD>
...
<FORM NAME="myForm" ACTION="/servlet/CmsController" METHOD="POST">
	...
	<input type=hidden name="id" value="<%=f.get("FileId")%>">
	<%
			...
			<td>
				<% 
					for (int i=0;i<cats.size();i++) {
						...
						<a href="/servlet/CmsController?action=removeFromCategory&catReturnTo=<%=ar.getValue("CatReturnTo")%>&id=<%=f.get("FileId")%>&catId=<%=cat.get("CategoryId")%>">[remove]</a>
						</font><br>
						<%
					}
				%>
	...
	<a href="/servlet/CmsController?action=deleteResource&id=<%=f.get("FileId")%>&catId=<%=ar.getValue("CatReturnTo")%>" onMouseOver="document.deletebutton.src='/images/delete_bon.gif';" onMouseOut="document.deletebutton.src='/images/delete_boff.gif';"><IMG  NAME="deletebutton" SRC="/images/delete_boff.gif" BORDER="0" ALIGN="TOP"></A> 
	<A HREF="/servlet/CmsController?action=fileInfo&catReturnTo=<%=ar.getValue("CatReturnTo")%>&id=<%=f.get("FileId")%>" onMouseOver="document.backbutton.src='/images/back_bon.gif';" onMouseOut="document.backbutton.src='/images/back_boff.gif';"><IMG  NAME="backbutton" SRC="/images/back_boff.gif" BORDER="0" ALIGN="TOP"></A>
	</center>
	<p>
</font>
</td></tr></table>
