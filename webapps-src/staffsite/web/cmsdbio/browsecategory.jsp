...
<center>
	<%
	if(!ar.getValue("CategoryId").equals("0")) {
		...
		<center>
		<p><A HREF="/servlet/CmsController?action=addToCategory&catReturnTo=<%=ar.getValue("CatReturnTo")%>&id=<%=ar.getValue("id")%>&catId=<%=ar.getValue("CategoryId")%>">[Select THIS Category]</A></p>
		</center>
		<%=box.printBottom()%>
		<br>
		<%
	...
	if (subCats.size()>0) {
		...
		<%
			...
			%>
				<img src="/images/blank.gif" height=1 width=20>
				<A HREF="/servlet/CmsController?action=browseCat&catReturnTo=<%=ar.getValue("CatReturnTo")%>&id=<%=ar.getValue("id")%>&catId=<%=subCat.get("CategoryId")%>">
