...
function openWin(input){ 
	if(input == 'addToCat'){
		window.open("/servlet/CmsController?action=browseCat&catReturnTo=moderate&id=<%=f.get("FileId")%>","Add","resizable=no,scrollbars=yes,width=400,height=500,top=100,left=100");
	} else if(input == 'replace'){
		window.open("/servlet/CmsController?action=showReplace&catReturnTo=moderate&id=<%=f.get("FileId")%>","ReplaceResource","resizable=no,scrollbars=yes,width=600,height=300,top=100,left=100");
	}
}
</SCRIPT>
</HEAD>
...
<FORM NAME="myForm" ACTION="/servlet/CmsController" METHOD="POST">
			...
			<td>
				<% 
					for (int i=0;i<cats.size();i++) {
						...
						<a href="/servlet/CmsController?action=removeFromCategory&catReturnTo=moderate&id=<%=f.get("FileId")%>&catId=<%=cat.get("CategoryId")%>">[remove]</a>
						</font><br>
						<%
					}
				%>
	...
	<a href="/servlet/CmsController?action=deleteResource&catReturnTo=moderate&id=<%=f.get("FileId")%>">[Delete]</a> | 
