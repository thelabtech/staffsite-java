...
function openWin(input){ 
	if(input == 'replace'){
		window.open("/servlet/CmsController?action=showReplace&id=<%=f.get("FileId")%>","ReplaceResource","resizable=no,scrollbars=yes,width=600,height=300,top=100,left=100");
	}
}
</SCRIPT>
</HEAD>
...
<table width="90%" align="center"><tr><td>
			...
			<td>
				<% 
					for (int i=0;i<cats.size();i++) {
						...
						%>
						<%=font%>
							<a href="/servlet/CmsController?action=browse&catId=<%=cat.get("CategoryId")%>">
								Home:<%=cat.get("Path")%>
							</a> 
						</font><br>
						<%
	...
	<table border=0 cellpadding=0 cellspacing=0>
		...
		<tr><td valign="top"><%=font%>FileId:</td><td><%=font%><%=f.get("FileId")%></td></tr>
	</table>
	<%=box.printBottom()%>
	...
	if(moderator.equals("true")) {
		%>
		<A HREF="/servlet/CmsController?action=edit&catReturnTo=<%=ar.getValue("CatReturnTo")%>&id=<%=f.get("FileId")%>" onMouseOver="document.editbutton.src='/images/edit_bon.gif';" onMouseOut="document.editbutton.src='/images/edit_boff.gif';"><IMG  NAME="editbutton" SRC="/images/edit_boff.gif" BORDER="0" ALIGN="TOP"></A>
		<%
	}
	%>
