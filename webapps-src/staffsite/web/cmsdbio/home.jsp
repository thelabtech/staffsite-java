...
function openWin(input){ 
	...
	if(input == 'addCat'){
		window.open("/servlet/CmsController?action=showAddCategory&catId=1","Add","resizable=no,scrollbars=yes,width=400,height=100,top=100,left=100");
	}
...
<!-- REMOVED DUE TO PDF ISSUE
			...
			<%//=box.printTop()%>
				...
				*/		%>
						<LI><A HREF="/servlet/CmsController?action=fileInfo&catReturnTo=0&id=<%//=top10.get("FileId")%>"><%//=top10.get("Title")%></A>
						<%
				//	}
				%>
				</OL></FONT>
...
-->
			...
			if(moderator.equals("true")) {
				%>
				<A HREF="/servlet/CmsController?action=moderate" onMouseOver="document.editorpagebutton.src='/images/moderate_bon.gif';" onMouseOut="document.editorpagebutton.src='/images/moderate_boff.gif';"><IMG  NAME="editorpagebutton" SRC="/images/moderate_boff.gif" BORDER="0" ALIGN="TOP"></A><BR>
