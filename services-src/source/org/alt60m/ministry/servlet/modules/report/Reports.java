...
public class Reports {
	...
	private static String  conditionsPortion(String type, String region, String periodEnd, String periodBegin, String strategyList, String localLevelId, String targetAreaID){
		String queryPortion="";
		queryPortion="WHERE ( true ";
		if (!strategyList.contains("CM")){
			queryPortion+=" and (ministry_targetarea.eventType is not null) and (not(ministry_targetarea.eventType <=> '' )) ";
		}
		if (!strategyList.contains("C2")){
			queryPortion+=" and (not(ministry_targetarea.eventType <=> 'C1') and not(ministry_targetarea.eventType <=> 'C2')) ";
		}
		...
		queryPortion+=	" and ministry_statistic.periodEnd < "+periodEnd+
			...
			{
				queryPortion+=" and (ministry_targetarea.region is not null) AND ((ministry_targetarea.region <> '')or(ministry_targetarea.eventType is not null)) ";
			}
	...
	private static String groupPortion(String type){ 
		...
		{
			return " GROUP BY substring(ministry_targetarea.eventType,1,1), ministry_targetarea.region ORDER BY substring(ministry_targetarea.eventType,1,1), ministry_targetarea.region";
		}
		else if(type.equals("targetarea"))
		{
	...
	private static String summingFieldsPortion(String type){
		...
		if (type.equals("targetarea")){					
			queryPortion+=" ministry_activity.status as status, "+
				...
				" MAX(ministry_targetarea.eventKeyID) as eventKeyID,"+
	...
	private static ReportRow resultSet2ReportRow(ResultSet sums, ResultSet demos)throws Exception 
	{
		...
		row.setEventKeyID(sums.getString("eventKeyID"));
		row.setEventType((sums.getString("eventType")+"").equals("C1")?"C2":((sums.getString("eventType")==null)?"Campus":sums.getString("eventType")));
	...
	private static Hashtable<String,String> eventTypes(){
		Hashtable<String,String>types=new Hashtable<String,String>();
		types.put("Campus", "Campus");
		types.put("", "Campus");
		types.put("C2", "Conferences");
		types.put("C1", "Conferences");
		types.put("SP", "Summer Projects");
		return types;
	}
	...
	public static Vector<ReportRow> getSuccessCriteriaReport(String type, String region, String strategyList, String periodEnd, String periodBegin, String localLevelId, String targetAreaId) throws Exception{
		try {		
			...
			if (sums.isBeforeFirst()){
					...
					if (sums.getString("rowid").equals(demos.getString("rowid"))){
						...
						if(!(row.getEventType()+"").equals(lastRow.getEventType()+"")){
							...
							if (!(lastRow.getRowid()+"").equals("")){
								...
								ReportRow eventBlock=new ReportRow();
								eventBlock.setFunction("eventBlockBottom");
								eventBlock.setEventType(lastRow.getEventType()+"");
							...
							eventTopper.setLabel(eventTypes().get(row.getEventType()));
				...
				ReportRow eventBlock=new ReportRow();
				eventBlock.setFunction("eventBlockBottom");
				eventBlock.setEventType(row.getEventType()+"");
				eventBlock.setLabel(eventTypes().get(row.getEventType()));
				report.add(eventBlock);
				runningTotal=new ReportRow();
	...
	private static StringBuffer getMusterTop(String type){
		...
		if (type.equals("movement")){
			renderedReport.append("<tr ><td colspan=\"2\" class=\"muster_cap_label\">Movements</td>");
		}
		else if (type.equals("location")){
			renderedReport.append("<tr ><td class=\"muster_cap_label\">Ministry Locations</td>");
		}
		else if (type.equals("teamorg")){
			renderedReport.append("<tr ><td class=\"muster_cap_label\">Missional Teams</td>");
		}
		else if (type.equals("teamgeo")){
			renderedReport.append("<tr ><td class=\"muster_cap_label\">Missional Teams</td>");
		}	
		renderedReport.append("<td class=\"muster_cap\">Region</td>");
		renderedReport.append("<td class=\"muster_cap\">City</td><td class=\"muster_cap\">State</td><td class=\"muster_cap\">Country</td>");
		if (type.equals("location")){
			renderedReport.append("<td class=\"muster_cap\">Enrollment</td>");
			} 
		if (type.equals("movement")){
			renderedReport.append("<td colspan=\"2\" class=\"muster_cap\">Missional Team</td>");
		}
	...
	private static StringBuffer getMusterBottom(String type, int rows, int secureRows, int enrollment){
		StringBuffer renderedReport=new StringBuffer();
		if (type.equals("movement")){
			renderedReport.append("<tr ><td colspan=\"2\" class=\"muster_cap_label\">"+org.alt60m.util.Toolbox.commatize(rows+"")+" Movements On Record<br>"+(secureRows>0?"<i>"+secureRows+" Sensitive Movements Not Displayed</i>":"")+"</td>");
		}
		else if (type.equals("location")){
			renderedReport.append("<tr ><td class=\"muster_cap_label\">"+org.alt60m.util.Toolbox.commatize(rows+"")+" Ministry Locations On Record<br>"+(secureRows>0?"<i>"+secureRows+" Sensitive Locations Not Displayed</i>":"")+"</td>");
		}
		else if (type.equals("teamorg")){
			renderedReport.append("<tr ><td class=\"muster_cap_label\">"+org.alt60m.util.Toolbox.commatize(rows+"")+" Missional Teams On Record</td>");
		}
		else if (type.equals("teamgeo")){
			renderedReport.append("<tr ><td class=\"muster_cap_label\">"+org.alt60m.util.Toolbox.commatize(rows+"")+" Missional Teams On Record</td>");
		}	
		...
		renderedReport.append("<td class=\"muster_cap\">City</td><td class=\"muster_cap\">State</td><td class=\"muster_cap\">Country</td>");
		if (type.equals("location")){
			renderedReport.append("<td class=\"muster_cap\">Enrollment <br>("+org.alt60m.util.Toolbox.commatize(enrollment+"")+" total)</td>");
			} 
		if (type.equals("movement")){
			renderedReport.append("<td colspan=\"2\" class=\"muster_cap\">Missional Team</td>");
		}
	...
	public  static StringBuffer renderMuster(StringBuffer renderedReport, boolean lighter, java.sql.ResultSet resultSet, String type, java.util.Vector<String> keys)throws Exception{
		...
		cellAlt="darker";
					...
					if(
						...
						renderedReport.append("</td><td class=\"muster_"+cellAlt +"\">"+(secure?"<i>":"")+""+org.alt60m.ministry.Regions.expandRegion(resultSet.getString("region")!=null?resultSet.getString("region"):"")+"</td>");
						renderedReport.append("<td class=\"muster_"+cell +"\">"+(secure?"<i>":"")+""+resultSet.getString("city")+"</td>");
						renderedReport.append("<td class=\"muster_"+cellAlt +"\">"+(secure?"<i>":"")+""+resultSet.getString("state")+"</td>");
						renderedReport.append("<td class=\"muster_"+cell +"\">"+(secure?"<i>":"")+""+resultSet.getString("country")+"</td>");
						if (type.equals("movement")){ 
						...
						if(type.equals("location")){
							renderedReport.append("<td class=\"muster_"+cellAlt +"\">"+(secure?"<i>":"")+""+org.alt60m.util.Toolbox.commatize(resultSet.getString("enrollment"))+"</td>");
						}
			renderedReport.append("</tr>");
					}else{
			secureRows++;
