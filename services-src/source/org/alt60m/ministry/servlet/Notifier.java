...
public class Notifier {
	...
	public Notifier() {
		getConfigProperties(org.alt60m.servlet.ObjectMapping.getConfigPath()
				+ "/hrtool_notification.properties");
		/*
		 * log.debug("***********************"); log.debug("WEBSITE_EMAIL_ADDR = " +
		 * WEBSITE_EMAIL_ADDR + "\nERROR_EMAIL = " + ERROR_EMAIL +
		 * "\nPIU_STAFF_SERVICES_EMAIL =
		 * "+PIU_STAFF_SERVICES_EMAIL+"\nACS_STAFF_SERVICES_EMAIL =
		 * "+ACS_STAFF_SERVICES_EMAIL); log.debug("***********************");
		 */
	}

	/**
	 * Static method that will take an unordered <CODE>Collection</CODE> of
	 * AuthorizationObjects and put them into a <CODE>List</CODE>, ordered by
	...
	/**
	 * This method both identifies the next Staff person that needs to authorize
	 * a change request, gets his/her email address, and sends an email
	 * notifying of a pending change that requires their authorization.
	 */
	public void notifyNextAuthorizor(StaffChangeRequest change)
			throws Exception {
		log.debug("Entering notifyNextAuthorizer");
		try {
			final String nextAuthorizorEmail;
			Staff updateStaff = change.getUpdateStaff(); // get staff record
															// for staff person
															// being updated
			List orderedAuths = orderAuthorizationsBySequence(change
					.getAuthorizations());
			// boolean region2region = false;
			String receivingRegion = null;

			// if we are moving from region to region, the hard part is getting
			// the
			// receiving region - need to iterate through the fieldchanges to
			// find
			...
			if (region == null || region.equals("")) // only get the region
														// from the change
														// request table if they
														// have no region
			{
			...
			// Now we need to look for the next Authorization Object that is
			// untouched
			while (lit.hasNext()) {
				auth = (Authorization) lit.next();
				if (auth.getAuthorized() == null
						|| auth.getAuthorized().equals("")) {
					// OK, now we have the next required authorization, need to
					// get
					...
					log
							.debug("** looking for auth for :" + region
									+ ": region");
					log.debug("*** sequence no " + auth.getSequence()
							+ " is unauthorized");
					break;
				}
			}

			// at this point it is either the campus or non-campus message that
			// needs to be sent

			// HRNC authorization...needs to be fake authorized and sent on to
			// next authorizor
			if (auth.getRole().equals(StaffChangeRequest.HR_NON_CAMPUS)) {
				HRTool hrt = new HRTool();
				hrt.authorize(change.getChangeRequestId(), auth
						.getAuthorizationId(), null, "StaffSite");

				// HRNC authorization...no more auths to go so apply changes and
				// send to Staff Services
				/*
				 * } else if
				 * (auth.getRole().equals(StaffChangeRequest.HR_NON_CAMPUS) &&
				 * auth.getSequence() == 3) {
				 * ApplyStaffChanges.applyChanges(change); //Really we should
				 * apply these changes someplace else log.debug("Could be
				 * emailing SS now, hopefully w/ a pending approval message");
				 * emailStaffServices(change.getChangeRequestId());
				 */// notifyNonCampus();
			} else if (auth.getRole()
					.equals(StaffChangeRequest.HR_REGIONAL_DIR)
					|| auth.getRole()
							.equals(StaffChangeRequest.HR_NATIONAL_DIR)) {
				log.debug("notifying a campus person from :" + region
						+ ": region");
				log.debug("using auth with seqno " + auth.getSequence());

				// If no region, automatically approve (since we're forcing them
				// to have a region now)
				if (auth.getRole().equals(StaffChangeRequest.HR_REGIONAL_DIR)
						&& auth.getSequence() == 1 && region.equals("")) {
					AuthorizeChangeRequest aci = new AuthorizeChangeRequest();
					aci.authorize(change.getChangeRequestId(), auth
							.getAuthorizationId(),
							"Automated Authorization due to null region.",
							"StaffSite");
				}

				// If a strange region, error out.
				if (!region.equals("GL") && !region.equals("GP")
						&& !region.equals("MA") && !region.equals("MS")
						&& !region.equals("NC") && !region.equals("NE")
						&& !region.equals("NW") && !region.equals("RR")
						&& !region.equals("SE") && !region.equals("SW")
						&& !region.equals("UM") && !region.equals("IN")) {
	...
	private void notifyNonCampus() {
		// here is where we compose and send the message to staff services
		// regarding this change
		log.debug("NOT IMPLEMENTED Notifying Non-Campus Person");
	}

	private void notifyCampus(StaffChangeRequest change, Staff updateStaff,
			Authorization auth, String region) throws Exception {
		...
		try {
			...
			List orderedAuths = orderAuthorizationsBySequence(change
					.getAuthorizations());
			if (orderedAuths.size() == 2
					&& auth.getSequence() == 1
					&& ((Authorization) orderedAuths.get(0)).getRole().equals(
							StaffChangeRequest.HR_REGIONAL_DIR)
					&& ((Authorization) orderedAuths.get(1)).getRole().equals(
							StaffChangeRequest.HR_REGIONAL_DIR)) {
				String receivingRegion = null;
				Collection c = change.getFieldChanges();
				Iterator it = c.iterator();
				...
				lookUpAuthEmail((Authorization) orderedAuths.get(1),
						receivingRegion);
			}
		} catch (AuthorizerNotFoundException e) {
			throw e;
	...
	private String lookUpAuthEmail(Authorization auth, String region)
			throws Exception {
	...
	private String lookupCampusEmail(Authorization auth, String region)
			throws AuthorizerNotFoundException, Exception {
		...
		/***********************************************************************
		 * NOTE: It is very important that this query returns only one record.
		 * If the returned collection contains more than one record, the SQL
		 * needs to be narrowed, or an array of strings created as the TO: field
		 * in the email.
		 **********************************************************************/
		if (c.size() > 0) {
			Iterator iter = c.iterator();
			nextAuthorizor = (Staff) iter.next();
			log.debug("Found " + nextAuthorizor.getLastName()
					+ " to authorize this request");
		} else {
			// If we have NO authorizor, we have got a problem.
			// Send an email to an admin to fix this data issue.
			StaffChangeRequest scr = new StaffChangeRequest(auth
					.getChangeRequestId());
	...
	/**
	 * Method to notify staff services of a personnel change that DOES NOT
	 * require authorization. This method will be called immediately after
	...
	public void emailStaffServices(Map formData) throws Exception {
		...
		if (changeType.equals(StaffChangeRequest.ADDRESS_CHANGE)) {
			body += "Change of Address for " + formData.get("FirstName") + " "
			...
			if (((String) formData.get("addrChangeType")).equals("Permanent")) {
				body += "Effective Date: "
						+ (String) formData.get("primaryStartDate") + "\n";
			} else {
				body += "Beginning: "
						+ (String) formData.get("primaryStartDate") + "\n";
			...
			if (formData.get("primaryEmpLocCity") != null) {
				body += "Primary Employment City: "
						+ formData.get("primaryEmpLocCity") + "\n";
			}
			if (formData.get("primaryEmpLocState") != null) {
				body += "Primary Employment State: "
						+ formData.get("primaryEmpLocState") + "\n";
			}
			if (formData.get("primaryEmpLocCountry") != null) {
				body += "Primary Employment Country: "
						+ formData.get("primaryEmpLocCountry") + "\n";
			}
			...
			body += "Marital Status Change for " + formData.get("FirstName")
					+ " " + formData.get("LastName") + "\n";
			body += "Account No: " + formData.get("AccountNo") + "\n\n";
			if (formData.get("fianceeFirstName") != null) {
				body += "Fiancee: " + formData.get("fianceeFirstName") + " "
						+ formData.get("fianceeMiddleName") + " "
						+ formData.get("fianceeLastName") + "\n";
			}
			if (((Boolean) formData.get("IsMale")).booleanValue()) {
				body += "Wife's married name: "
						+ formData.get("spouseFirstName") + " "
						+ formData.get("spouseMiddleName") + " "
						+ formData.get("spouseLastName") + "\n";
			} else {
				body += "Wife's married name: " + formData.get("firstName")
			...
			if (formData.get("spouseAccountNo") != null) {
				body += "Fiancee is staff, account number: "
						+ formData.get("spouseAccountNo") + "\n";
			} else if (formData.get("fianceeJoinStaffDate") != null) {
				body += formData.get("fianceeFirstName")
						+ "will be joining Campus Crusade Staff on "
						+ formData.get("fianceeJoinStaffDate") + ".\n";
			}
			if (formData.get("joiningNS") != null
					&& ((String) formData.get("joiningNS")).equals("true")) {
				body += "You will be joining National Staff." + "\n";
			}
			if (formData.get("joiningNS") != null
					&& ((String) formData.get("isFianceeJoiningNS"))
							.equals("true")) {
				body += formData.get("fianceeFirstName")
						+ " will be joining National Staff." + "\n";
			}
		} else if (changeType.equals(StaffChangeRequest.DEPENDENT_CHANGE)) {
			body += "Dependent Change for " + formData.get("FirstName") + " "
					+ formData.get("LastName") + "\n";
			body += "Account No: " + formData.get("AccountNo") + "\n\n";
	...
	/**
	 * Method to notify staff services of a change that DID require
	 * authorization. This notification happens AFTER all required approvals
	 * have been met.
	 */
	public void emailStaffServices(String changeRequestId) throws Exception {
		...
		String body = new String(
				"The staff website has received an administrative change of status.\n\n");
	...
	private String composeJobChangeBody(Staff staff, StaffChangeRequest stf)
			throws Exception {
		String body = new String();
		Collection fields = stf.getFieldChanges();
		Iterator iter = fields.iterator();
		body += "Job Change Request for " + staff.getFirstName() + " "
				+ staff.getLastName() + "\n";
		body += "Account No: " + staff.getAccountNo() + "\n";
		body += "Effective Date: " + stf.getEffectiveDate() + "\n\n";
		while (iter.hasNext()) {
			// This field copying part in each compose method could be all
			// rolled together possibly
	...
	private String composeLOABody(Staff staff, StaffChangeRequest stf)
			throws Exception {
		String body = new String();
		Collection fields = stf.getFieldChanges();
		Iterator iter = fields.iterator();

		body += "Leave of Absence for " + staff.getFirstName() + " "
				+ staff.getLastName() + "\n";
		body += "Account No: " + staff.getAccountNo() + "\n\n";
		while (iter.hasNext()) {
			FieldChange nextChange = (FieldChange) iter.next();
			if (nextChange.getField().equals("loaStartDate")) { // leave of
																// absence
																// fields
	...
	private String composeResignationBody(Staff staff, StaffChangeRequest stf)
			throws Exception {
		String body = new String();
		Collection fields = stf.getFieldChanges();
		Iterator iter = fields.iterator();

		body += "Resignation for " + staff.getFirstName() + " "
				+ staff.getLastName() + "\n";
		body += "Account No: " + staff.getAccountNo() + "\n\n";
		while (iter.hasNext()) {
			FieldChange nextChange = (FieldChange) iter.next();
			if (nextChange.getField().equals("resignationDate")) { // resignation
																	// fields
				...
				body += "Severance Pay Requested: " + nextChange.getNewValue()
						+ " months.\n";
			} else if (nextChange.getField().equals("contributionsToOtherAcct")) {
			} else if (nextChange.getField().equals("contributionsToAcntNo")) {
				body += "Contributions to go to Account number: "
						+ nextChange.getNewValue() + "\n";
	...
	private String appendAuthorizations(StaffChangeRequest stf)
			throws Exception {
		String body = new String();
		...
		for (int i = 0; i < orderedAuths.size(); i++) {
			...
			log("a is " + a.getAuthorizationId() + " :" + a.getAuthorized()
					+ ": " + a.getAuthDate());
			if (((Authorization) orderedAuths.get(i)).getAuthorized().equals(
					"Y")) {
				Staff authorizer = ((Authorization) orderedAuths.get(i))
						.getAuthorizedBy();
				if (authorizer == null) {
				...
				} // end of if ((Authorization) orderedAuths.get(i)).getRole()
					// == null)

				if ((authorizer != null)
						&& (((Authorization) orderedAuths.get(i)).getRole() != null)) {
					body += "Approved by: " + authorizer.getPreferredName()
							+ " " + authorizer.getLastName() + " "
							+ ((Authorization) orderedAuths.get(i)).getRole()
							+ " " + authorizer.getAccountNo() + "\n";
				}
			} else {
				log("authorzed is NOT 'Y'");
				log(((Authorization) orderedAuths.get(i)).getAuthorizationId()
						+ " is :"
						+ ((Authorization) orderedAuths.get(i)).getAuthorized()
						+ ": "
						+ ((Authorization) orderedAuths.get(i)).getAuthDate());
			} // end of else
	...
	private void getConfigProperties(String propsFilePath) {
		try {
			...
			PIU_STAFF_SERVICES_EMAIL = p
					.getProperty("piu_staff_services_email");
			ACS_STAFF_SERVICES_EMAIL = p
					.getProperty("acs_staff_services_email");
			ACS_RESIGN_STAFF_SERVICES_EMAIL = p
					.getProperty("acs_resign_staff_services_email");
	...
	/**
	 * This should be a generic email address, something like
	 * staff.services@uscm.org All notifications for staff services go to this
	 * address.
	 */
	...
	/**
	 * In the case where the authorizing authority can't be found, an email is
	 * sent here notifying recipient of the problem.
	 */
