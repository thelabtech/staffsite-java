...
/**
 * @persistent 
 * @table si_SIReference_Archive2003
 * @cache-type none
 * @access shared
 * @key-generator IDENTITY
 */
public class SIReference_Archive2003 extends SIReference_Parent {
	public final static String DEFAULT_FROM_EMAIL = "help@campuscrusadeforchrist.com";
    public static final String APPLICATIONCLASS = "org.alt60m.hr.si.model.dbio.SIApplication_Archive2003";
	public static final String PERSONCLASS = "org.alt60m.hr.si.model.dbio.SIPerson_Archive2003";
	public static final String REFERENCECLASS = "org.alt60m.hr.si.model.dbio.SIReference_Archive2003";
	
	public SIReference_Archive2003() {
	...
	public SIReference_Archive2003(String id) {
		// Archive class allows inheritance through polymorphism & upcasting
		referenceID= (new Integer(id)).intValue();
		select();
	}
	public SIReference_Archive2003(int id) {
		// Archive class allows inheritance through polymorphism & upcasting
		referenceID= id;
		select();
	}
	...
	public void localinit() {
		String table = "hr_si_Reference_Archive2003";
...
**/
	...
	public boolean sendEmailInvite(String serverName, SIPerson person) {
		try {
			...
			if (emailContactText.length() != 0){
				...
				text += "This email is to notify you that we have sent an email on your behalf to " + refFullName + " inviting him or her to submit a reference for your participation on STINT.  The email was sent to:" + this.getCurrentEmail() + ".\n\n";
				text += "If the email address is invalid or the message is returned for any reason, you will receive the returned email.  If this happens, 1) please login to your STINT application, 2) go to the References page, 3) correct the reference's email address, and 4) click the 'Send Email Invitation' button.\n\n";
	...
	public boolean sendEmailRefComplete() {
		try {
			...
			if (applicantEmailAddress == null  ||  applicantEmailAddress.trim().equals("")){
				...
				msg.setTo(applicantEmailAddress);
				msg.setFrom(DEFAULT_FROM_EMAIL);
				msg.setSubject("Internship Reference Complete for " + applicantFullName);
				msg.setBody(text);
				msg.send();
				return true;
			}
	...
	public boolean sendEmailThankYou() {
		try {
			...
			msg.setTo(this.getCurrentEmail());
			msg.setFrom(DEFAULT_FROM_EMAIL);
			msg.setSubject("Thank You for Completing a Reference for " + applicantFullName);
			msg.setBody(text);
			msg.send();
			return true;
		} catch(Exception e) {
